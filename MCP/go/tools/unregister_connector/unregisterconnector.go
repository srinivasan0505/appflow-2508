...
func UnregisterconnectorHandler(cfg *config.APIConfig) func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
	return func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
		...
		var requestBody models.UnregisterConnectorrequest
		...
		url := fmt.Sprintf("%s/unregister-connector", cfg.BaseURL)
		...
		var result models.UnregisterConnectorResponse
...
func CreateUnregisterconnectorTool(cfg *config.APIConfig) models.Tool {
	tool := mcp.NewTool("post_unregister-connector",
		mcp.WithDescription("Unregisters the custom connector registered in your account that matches the connector label provided in the request."),
		...
		mcp.WithBoolean("forceDelete", mcp.Description("Input parameter: Indicates whether Amazon AppFlow should unregister the connector, even if it is currently in use in one or more connector profiles. The default value is false.")),
		mcp.WithString("connectorLabel", mcp.Required(), mcp.Description("Input parameter: The label of the connector. The label is unique for each <code>ConnectorRegistration</code> in your Amazon Web Services account.")),
	)

	return models.Tool{
		Definition: tool,
		Handler:    UnregisterconnectorHandler(cfg),
	}
}
