...
func DescribeconnectorprofilesHandler(cfg *config.APIConfig) func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
	return func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
		...
		var requestBody models.DescribeConnectorProfilesrequest
		...
		url := fmt.Sprintf("%s/describe-connector-profiles%s", cfg.BaseURL, queryString)
		...
		var result models.DescribeConnectorProfilesResponse
...
func CreateDescribeconnectorprofilesTool(cfg *config.APIConfig) models.Tool {
	tool := mcp.NewTool("post_describe-connector-profiles",
		mcp.WithDescription("<p> Returns a list of <code>connector-profile</code> details matching the provided <code>connector-profile</code> names and <code>connector-types</code>. Both input lists are optional, and you can use them to filter the result. </p> <p>If no names or <code>connector-types</code> are provided, returns all connector profiles in a paginated form. If there is no match, this operation returns an empty list.</p>"),
		...
		mcp.WithString("connectorLabel", mcp.Description("Input parameter: The name of the connector. The name is unique for each <code>ConnectorRegistration</code> in your Amazon Web Services account. Only needed if calling for CUSTOMCONNECTOR connector type/.")),
		mcp.WithArray("connectorProfileNames", mcp.Description("Input parameter:  The name of the connector profile. The name is unique for each <code>ConnectorProfile</code> in the Amazon Web Services account. ")),
	...
	return models.Tool{
		Definition: tool,
		Handler:    DescribeconnectorprofilesHandler(cfg),
	}
}
