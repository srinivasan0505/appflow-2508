...
func DeleteflowHandler(cfg *config.APIConfig) func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
	return func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
		...
		var requestBody models.DeleteFlowrequest
		...
		url := fmt.Sprintf("%s/delete-flow", cfg.BaseURL)
		...
		var result models.DeleteFlowResponse
...
func CreateDeleteflowTool(cfg *config.APIConfig) models.Tool {
	tool := mcp.NewTool("post_delete-flow",
		mcp.WithDescription(" Enables your application to delete an existing flow. Before deleting the flow, Amazon AppFlow validates the request by checking the flow configuration and status. You can delete flows one at a time. "),
		...
		mcp.WithBoolean("forceDelete", mcp.Description("Input parameter:  Indicates whether Amazon AppFlow should delete the flow, even if it is currently in use. ")),
