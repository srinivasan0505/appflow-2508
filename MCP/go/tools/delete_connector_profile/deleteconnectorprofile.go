...
func DeleteconnectorprofileHandler(cfg *config.APIConfig) func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
	return func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
		...
		var requestBody models.DeleteConnectorProfilerequest
		...
		url := fmt.Sprintf("%s/delete-connector-profile", cfg.BaseURL)
		...
		var result models.DeleteConnectorProfileResponse
...
func CreateDeleteconnectorprofileTool(cfg *config.APIConfig) models.Tool {
	tool := mcp.NewTool("post_delete-connector-profile",
		mcp.WithDescription(" Enables you to delete an existing connector profile. "),
		...
		mcp.WithString("connectorProfileName", mcp.Required(), mcp.Description("Input parameter:  The name of the connector profile. The name is unique for each <code>ConnectorProfile</code> in your account. ")),
		mcp.WithBoolean("forceDelete", mcp.Description("Input parameter:  Indicates whether Amazon AppFlow should delete the profile, even if it is currently in use in one or more flows. ")),
	)

	return models.Tool{
		Definition: tool,
		Handler:    DeleteconnectorprofileHandler(cfg),
	}
}
