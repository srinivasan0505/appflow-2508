...
func DescribeconnectorentityHandler(cfg *config.APIConfig) func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
	return func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
		...
		var requestBody models.DescribeConnectorEntityrequest
		...
		url := fmt.Sprintf("%s/describe-connector-entity", cfg.BaseURL)
		...
		var result models.DescribeConnectorEntityResponse
...
func CreateDescribeconnectorentityTool(cfg *config.APIConfig) models.Tool {
	tool := mcp.NewTool("post_describe-connector-entity",
		mcp.WithDescription(" Provides details regarding the entity used with the connector, with a description of the data model for each field in that entity. "),
		...
		mcp.WithString("connectorProfileName", mcp.Description("Input parameter:  The name of the connector profile. The name is unique for each <code>ConnectorProfile</code> in the Amazon Web Services account. ")),
		mcp.WithString("connectorType", mcp.Description("Input parameter:  The type of connector application, such as Salesforce, Amplitude, and so on. ")),
		mcp.WithString("apiVersion", mcp.Description("Input parameter: The version of the API that's used by the connector.")),
		mcp.WithString("connectorEntityName", mcp.Required(), mcp.Description("Input parameter:  The entity name for that connector. ")),
	)

	return models.Tool{
		Definition: tool,
		Handler:    DescribeconnectorentityHandler(cfg),
	}
}
