...
func DescribeflowexecutionrecordsHandler(cfg *config.APIConfig) func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
	return func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
		...
		var requestBody models.DescribeFlowExecutionRecordsrequest
		...
		url := fmt.Sprintf("%s/describe-flow-execution-records%s", cfg.BaseURL, queryString)
		...
		var result models.DescribeFlowExecutionRecordsResponse
...
func CreateDescribeflowexecutionrecordsTool(cfg *config.APIConfig) models.Tool {
	tool := mcp.NewTool("post_describe-flow-execution-records",
		mcp.WithDescription(" Fetches the execution history of the flow. "),
		...
		mcp.WithNumber("maxResults", mcp.Description("Input parameter:  Specifies the maximum number of items that should be returned in the result set. The default for <code>maxResults</code> is 20 (for all paginated API operations). ")),
		mcp.WithString("nextToken", mcp.Description("Input parameter:  The pagination token for the next page of data. ")),
	)

	return models.Tool{
		Definition: tool,
		Handler:    DescribeflowexecutionrecordsHandler(cfg),
	}
}
