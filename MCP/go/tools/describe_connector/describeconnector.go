...
func DescribeconnectorHandler(cfg *config.APIConfig) func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
	return func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
		...
		url := fmt.Sprintf("%s/describe-connector", cfg.BaseURL)
...
func CreateDescribeconnectorTool(cfg *config.APIConfig) models.Tool {
	tool := mcp.NewTool("post_describe-connector",
		mcp.WithDescription("Describes the given custom connector registered in your Amazon Web Services account. This API can be used for custom connectors that are registered in your account and also for Amazon authored connectors."),
		...
		mcp.WithString("connectorType", mcp.Required(), mcp.Description("Input parameter: The connector type, such as CUSTOMCONNECTOR, Saleforce, Marketo. Please choose CUSTOMCONNECTOR for Lambda based custom connectors.")),
