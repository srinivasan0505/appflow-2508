...
func CreateTagresourceTool(cfg *config.APIConfig) models.Tool {
	tool := mcp.NewTool("post_tags_resourceArn",
		mcp.WithDescription(" Applies a tag to the specified flow. "),
		...
		mcp.WithString("resourceArn", mcp.Required(), mcp.Description(" The Amazon Resource Name (ARN) of the flow that you want to tag. ")),
		mcp.WithObject("tags", mcp.Required(), mcp.Description("Input parameter:  The tags used to organize, track, or control access for your flow. ")),
	)
