...
func CreateUntagresourceTool(cfg *config.APIConfig) models.Tool {
	tool := mcp.NewTool("delete_tags_resourceArn#tagKeys",
		mcp.WithDescription(" Removes a tag from the specified flow. "),
		...
		mcp.WithString("resourceArn", mcp.Required(), mcp.Description(" The Amazon Resource Name (ARN) of the flow that you want to untag. ")),
		mcp.WithArray("tagKeys", mcp.Required(), mcp.Description(" The tag keys associated with the tag that you want to remove from your flow. ")),
	)
