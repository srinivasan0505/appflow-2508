...
func CreateListtagsforresourceTool(cfg *config.APIConfig) models.Tool {
	tool := mcp.NewTool("get_tags_resourceArn",
		mcp.WithDescription(" Retrieves the tags that are associated with a specified flow. "),
		...
		mcp.WithString("resourceArn", mcp.Required(), mcp.Description(" The Amazon Resource Name (ARN) of the specified flow. ")),
	)
