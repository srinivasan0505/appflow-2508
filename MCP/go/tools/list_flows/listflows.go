...
func ListflowsHandler(cfg *config.APIConfig) func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
	return func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
		...
		var requestBody models.ListFlowsrequest
		...
		url := fmt.Sprintf("%s/list-flows%s", cfg.BaseURL, queryString)
		...
		var result models.ListFlowsResponse
...
func CreateListflowsTool(cfg *config.APIConfig) models.Tool {
	tool := mcp.NewTool("post_list-flows",
		mcp.WithDescription(" Lists all of the flows associated with your account. "),
		...
		mcp.WithString("nextToken", mcp.Description("Input parameter:  The pagination token for next page of data. ")),
		mcp.WithNumber("maxResults", mcp.Description("Input parameter:  Specifies the maximum number of items that should be returned in the result set. ")),
	)

	return models.Tool{
		Definition: tool,
		Handler:    ListflowsHandler(cfg),
	}
}
