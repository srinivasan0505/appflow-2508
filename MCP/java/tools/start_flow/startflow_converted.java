/**
 * MCP Server function for Activates an existing flow. For on-demand flows, this operation runs the flow immediately. For schedule and event-triggered flows, this operation activates the flow.
 */
...
class Post_Start_FlowMCPTool {
    
    public static Function<MCPServer.MCPRequest, MCPServer.MCPToolResult> getPost_Start_FlowHandler(MCPServer.APIConfig config) {
        return (request) -> {
            try {
        ...
        if (args.containsKey("flowName")) {
            queryParams.add("flowName=" + args.get("flowName"));
                ...
                String url = config.getBaseUrl() + "/api/v2/post_start_flow" + queryString;
    ...
    public static MCPServer.Tool createPost_Start_FlowTool(MCPServer.APIConfig config) {
        ...
        return new MCPServer.Tool(definition, getPost_Start_FlowHandler(config));
    }
