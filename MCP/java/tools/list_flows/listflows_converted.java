/**
 * MCP Server function for Lists all of the flows associated with your account.
 */
...
class Post_List_FlowsMCPTool {
    
    public static Function<MCPServer.MCPRequest, MCPServer.MCPToolResult> getPost_List_FlowsHandler(MCPServer.APIConfig config) {
        return (request) -> {
            try {
        ...
        if (args.containsKey("nextToken")) {
            queryParams.add("nextToken=" + args.get("nextToken"));
                ...
                String url = config.getBaseUrl() + "/api/v2/post_list_flows" + queryString;
    ...
    public static MCPServer.Tool createPost_List_FlowsTool(MCPServer.APIConfig config) {
        ...
        return new MCPServer.Tool(definition, getPost_List_FlowsHandler(config));
    }
